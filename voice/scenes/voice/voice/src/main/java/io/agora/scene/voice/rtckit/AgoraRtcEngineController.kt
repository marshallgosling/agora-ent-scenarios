...
class AgoraRtcEngineController {
    ...
    private var isMixing:Boolean = false
    ...
    fun mixerMusic(filePath: String, loop: Boolean, cycle: Int, pos: Int, volume: Int = 50) {
        rtcEngine?.startAudioMixing(filePath, loop, cycle, pos)
        rtcEngine?.adjustAudioMixingVolume(volume);
        isMixing = true;
    }

    fun updateMixerVolume(volume: Int) {
        if (isMixing) rtcEngine?.adjustAudioMixingVolume(volume);
    }
